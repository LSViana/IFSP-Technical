...
public class Vector<X> implements VectorInterface<X> {
    private X[] data;
    private int size;
    public Vector() { this(16); }
    public Vector(int initialCapacity) {
        size = 0;
        data = (X[]) new Object[initialCapacity];
    }
    // Interface methods
    @Override
    ...
    private void ensureCapacity() {
        if(size == data.length) {
            X[] fresh = (X[]) new Object[data.length * 2];
            copyArray(data, fresh);
            data = fresh;
        }
    }
    private void copyArray(X[] old, X[] fresh) {
        for (int i = 0; i < size; i++) {
            fresh[i] = old[i];
        }
    }
